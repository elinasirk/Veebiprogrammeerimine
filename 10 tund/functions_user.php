...
function saveNewPassword($newPassword){
	$notice = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("INSERT INTO vpusers (newPassword) VALUES(?)");
	echo $conn->error;
	...
	if($stmt->execute()){
		$notice = "Parooli vahetamine õnnestus!";
	}	else {
		$notice = "Parooli vahetamsiel tekkis tehniline tõrge: "  .$stmt->error;
	}
	$stmt->close();
	$conn->close();
	return $notice;
